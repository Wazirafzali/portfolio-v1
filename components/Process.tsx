...
export default function Process() {
  return <section id="process" className="studio-section process-section"><div className="studio-container"><div className="section-heading"><div><p className="eyebrow">03 / HOW WE WORK</p><h2>Good work starts<br /><em>with a clear process.</em></h2></div><p>One project contact. Shared milestones. Enough structure to keep things moving, and room for a real conversation.</p></div><div className="process-grid">{steps.map(step=><article key={step.number}><span className="process-number">{step.number}<i /></span><h3>{step.title}</h3><p>{step.description}</p><span className="process-output">{step.output}</span></article>)}</div></div></section>;
}
